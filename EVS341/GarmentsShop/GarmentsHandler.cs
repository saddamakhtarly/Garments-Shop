...
namespace EVS341.GarmentsShop
{
    public class GarmentsHandler
    {
        ...
        public List<Product> GetLatestProducts(int qty)
        {
            using (GarmentsContext context = new GarmentsContext())
            {
                return (from p in context.Products
                               ...
                               .Include(p => p.SizesOffered)
                               .Include(p => p.SubCategory.Category.Department)
                        orderby p.LaunchingDate
                        select p).Take(qty).ToList();
            }
        }

        public List<Product> GetFeaturedProducts(int qty)
        {
            using (GarmentsContext context = new GarmentsContext())
            {
                return (from p in context.Products
        ...
        public void DeleteOrder(int id)
        {
            using (GarmentsContext context = new GarmentsContext())
            {
                PlaceOrder found = context.PlaceOrders.Find(id);
                context.PlaceOrders.Remove(found);
                context.SaveChanges();
            }
        }
